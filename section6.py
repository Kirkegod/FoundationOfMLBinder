...
def data_likelihood_unlabeled(model_params, X_train_u):
    ...
    # Take the log after summing over classes, then sum over observations
    likelihood = np.sum(np.log(likelihood))

    return likelihood


def check_convergence(prev_likelihood, new_likelihood, threshold=1e-6):
    """Check convergence of EM algorithm
    prev_loss: (log) likelihood prior to parameter update
    new_loss:  (log) likelihood after parameter update
    threshold: maximum change in likelihood to stop algorithm

    returns: True if change in likelihood is smaller than threshold, False otherwise
    """
