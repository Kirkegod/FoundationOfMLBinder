...
def load_classification_data():
    ...
    train_data = np.load("data/section4/class_train_data.npy")
    X_train = train_data[:, :2]
    y_train = train_data[:, 2]

    # Load classification test/validation data
    test_data = np.load("data/section4/class_test_data.npy")
    X_test = test_data[:, :2]
    y_test = test_data[:, 2]
...
def load_classification_models():
    model_1 = pickle.load(open("data/section4/model_0", 'rb'))
    model_2 = pickle.load(open("data/section4/model_1", 'rb'))
    model_3 = pickle.load(open("data/section4/model_2", 'rb'))
